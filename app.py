...
def edit_user(id):
    ...
    return render_template('edit_user.html',role=role,user_profile=user_profile,user=user, messages=[msg])
...
def all_users():
    ...
    role=user_profile.role.value
    return render_template("user-list.html",role=role,user_profile=user_profile,all_users=all_users)
...
def super_admin():
    ...
    return render_template("superadmin_dashboard.html",role=role,user_profile=user_profile,all_users=all_users)
...
def super_admin_form_a():
    ...
    if forms_list:
        ...
        return render_template("superadmin_dashboard.html",role=role,forma=forma, **context)
    else:
        return render_template("superadmin_dashboard.html",role=role)
### Ploting form B
...
def super_admin_form_b():
    ...
    if forms_list:
        ...
        return render_template("superadmin_dashboard.html",role=role,formb=formb, **context)
...
def super_admin_form_c():
    ...
    if forms_list:
        ...
        return render_template("superadmin_dashboard.html",role=role,formc=formc, **context)
    else:
        return render_template("superadmin_dashboard.html",role=role)
...
def super_admin_monitoring_page_a():
    ...
    return render_template('super_admin_monitoring_page_a.html',role=role,current_time=current, forms_list=forms_list) #jsonify(forms_list) 
...
def super_admin_monitoring_page_b():
    ...
    return render_template('super_admin_monitoring_page_b.html',role=role,current_time=current, forms_list=forms_list) #jsonify(forms_list) 
...
def super_admin_monitoring_page_c():
    ...
    return render_template('super_admin_monitoring_page_c.html',role=role,current_time=current, forms_list=forms_list) #jsonify(forms_list) 
...
def submit_form_a_requirements():
    if request.method == 'POST':
        try:
            ...
            if db_session.query(FormB).filter_by(user_id=user_id).first() or \
            ...
            # File saving helper
            def save_file(file):
                """Save a single FileStorage object, return relative path."""
            ...
            if needs_permission:
                ...
                permission_paths = [
                    path for file in uploaded_files
                    if (path := save_file(file)) is not None
                ]
                if permission_paths:
                    permission_letter = ','.join(permission_paths)

            # Handle other file uploads (single files)
            research_tools_path = save_file(request.files.get('research_tools_path'))
            prior_clearance = save_file(request.files.get('prior_clearance')) if has_clearance else None
            prior_clearance1 = save_file(request.files.get('prior_clearance1')) if company_requires_jbs else None
            proposal_path = save_file(request.files.get('proposal_path'))
            impact_assessment_path = save_file(request.files.get('impact_assessment_path'))
            participation_info_sheet = save_file(request.files.get('participation_info_sheet'))
            ...
            if form:
                ...
                form.participation_info_sheet=participation_info_sheet
            else:
                form = FormARequirements(
                    ...
                    participation_info_sheet=participation_info_sheet,
                )
                db_session.add(form)
...
def form_a_sec4():
    ...
    if request.method == 'POST':
        ...
        if secondary_data == 'Yes':
            ...
            form.personal_info=request.form.get('personal_info')
            ...
            form.methodology_alignment=request.form.get('methodology_alignment')
    ...
    if form_name=="FORM A":
        ...
        if request.method=="POST":
            ...
            if reviewers:
                ...
                forma.reviewer_name1=id_of_reviewers[0]
                forma.reviewer_name2=id_of_reviewers[1]
            forma.review_date=request.form.get('review_date')
            forma.review_form_comments=request.form.get('additional_comments')
            forma.supervisor_form_status=request.form.get('recommendation')
        ...
        if request.method=="POST":
            ...
            if reviewers:
                ...
                formb.reviewer_name1=id_of_reviewers[0]
                formb.reviewer_name2=id_of_reviewers[1]
            formb.review_date=request.form.get('review_date')
            formb.review_form_comments=request.form.get('additional_comments')
            formb.supervisor_form_status=request.form.get('recommendation')
        ...
        if request.method=="POST":
            ...
            if Assigned_reviewer:
                ...
                formc.reviewer_name1=id_of_reviewers[0]
                formc.reviewer_name2=id_of_reviewers[1]
            formc.review_date=request.form.get('review_date')
            formc.review_form_comments=request.form.get('additional_comments')
            formc.supervisor_form_status=request.form.get('recommendation')
